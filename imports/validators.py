...
class ZippedShapefilesValidator:
    messages = {
        ...
        'fields_not_allowed': _('Fields not allowed: %(fields)s. Allowed shapefile fields are: %(allowed_fields)s.'),
        'feature_classes_not_exist': _('Feature classes do not exist: %(feature_classes)s.'),
    }
    ...
    def _validate_fields(self, shapefile_reader):
        shapefile_fields = [field[0] for field in shapefile_reader.fields[1:]]  # first field reserved as deletion flag
    ...
    def _validate_feature_classes(self, shapefile_reader):
        ...
        feature_classes = {record[feature_class_column_index] for record in shapefile_reader.iterRecords()}
        db_feature_classes = set(
            FeatureClass.objects.filter(id__in=feature_classes).values_list('id', flat=True)
        )
        ...
        if missing_feature_classes:
            raise ValidationError(
                ...
                params={
                    'feature_classes': ', '.join(missing_feature_classes)
                }
            )
