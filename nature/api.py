...
class ConservationProgrammeSerializer(ProtectedHyperlinkedModelSerializer):
    protected_features = SpanOneToOneProtectedHyperlinkedRelatedField(
        many=True,
        source="protections",
...
class SquareSerializer(ProtectedHyperlinkedModelSerializer):
    feature = SpanOneToOneProtectedHyperlinkedRelatedField(
        source="id", view_name="feature-detail", queryset=Feature.objects.all()
    )
...
class FeatureLinkSerializer(serializers.ModelSerializer):
    ...
    feature = SpanOneToOneProtectedHyperlinkedRelatedField(
        view_name="feature-detail", queryset=Feature.objects.all()
    )
...
class TransactionFeatureSerializer(ProtectedHyperlinkedModelSerializer):
    class Meta:
        ...
        fields = ("id", "url", "feature", "transaction")
...
class FeatureViewSet(ProtectedViewSet):
    queryset = Feature.objects.all().annotate(
        geom=Transform("geometry", 4326)
    )  # display coordinates in WGS84
...
class TransactionFeatureViewSet(ProtectedViewSet):
    queryset = TransactionFeature.objects.all()
...
router.register(r"transaction_feature", TransactionFeatureViewSet)
router.register(r"protection_criterion", ProtectionCriterionViewSet)
router.register(
    r"protection_conservation_programme", ProtectionConservationProgrammeViewSet
)
